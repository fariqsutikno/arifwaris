...
- [ ] **Step 1: Ekspor dari engine** — tambahkan ke `packages/engine/src/index.ts`:
...
- [ ] **Step 6: `src/main.tsx` dan `src/Aplikasi.tsx` sementara**
...
- [ ] **Step 7: Smoke test yang juga membuktikan engine bisa diimpor di Vite** — `src/__tests__/asap.test.tsx`:
...
- [ ] **Step 1: Tulis test yang gagal** — `src/__tests__/checklist.test.ts`:
...
- [ ] **Step 3: Implementasi** — `src/checklist.ts`:
...
- [ ] **Step 1: Test gagal** — `src/__tests__/kasus.test.ts`:
...
- [ ] **Step 3: Implementasi** — `src/kasus.ts`:
...
- [ ] **Step 1: Test gagal** — `src/__tests__/integrasi.test.ts`. Bandingkan dengan menjalankan engine pada fixture aslinya (id orang berbeda, jadi yang dibandingkan saham per kunci ahli waris dan jami'ah):
...
- [ ] **Step 1: Salin CSS dari design system.** Unduh `project/components/bundle.css` dari artifact `https://claude.ai/artifact/TAMrFT8QhPmBgB7Ho1gA43` ke `src/gaya/komponen.css` apa adanya (baris pertama `body{…}` dipertahankan). Tambahkan di akhir file gaya layar:
...
- [ ] **Step 2: `src/gaya/token.css`** — dari `tokens.json` design system (light default, dark lewat `prefers-color-scheme`):
...
- [ ] **Step 3: Test gagal** — `src/__tests__/komponen.test.tsx`:
...
- [ ] **Step 4: `src/ui/komponen.tsx`** — port 1:1 dari `bundle.js`, nama Indonesia:
...
- [ ] **Step 1: Test gagal** — `src/__tests__/keadaan.test.ts`:
...
- [ ] **Step 4: `src/layar/Wizard.tsx`** — kerangka + langkah 1–3; langkah 4 & 5 diimpor dari Task 7 (untuk sementara buat file stub `LangkahAhliWaris.tsx`/`LangkahKondisi.tsx` yang merender `<p>Segera</p>` dengan signature di Task 7 supaya build lolos):
...
- [ ] **Step 1: Test gagal** — `src/__tests__/langkah.test.tsx`:
...
- [ ] **Step 3: `src/layar/LangkahAhliWaris.tsx`**
...
- [ ] **Step 4: `src/layar/LangkahKondisi.tsx`**
...
- [ ] **Step 1: Test gagal** — `src/__tests__/hasil.test.tsx`:
...
- [ ] **Step 3: `src/layar/Penjelasan.tsx`**
...
- [ ] **Step 5: `src/layar/ModeBelajar.tsx`**
...
- [ ] **Step 1: Ganti smoke test dengan alur penuh** — `src/__tests__/asap.test.tsx`:
...
- [ ] **Step 5: `src/main.tsx`** — tambahkan impor CSS di atas:
...
- [ ] **Step 6: Jalankan semua test + build**
