 ...
 */
export function isUmariyyatain(efektif: AhliWaris[], ikhwahCount: number): boolean {
  ...
  return onlyThese && banyaknya(efektif, ['SUAMI', 'ISTRI']) > 0 && banyaknya(efektif, ['AYAH']) === 1
    && banyaknya(efektif, ['IBU']) === 1 && ikhwahCount < 2;
}
 ...
 */
export function isMusyarrakah(efektif: AhliWaris[]): boolean {
 ...
 */
export function isAkdariyyah(efektif: AhliWaris[]): boolean {
