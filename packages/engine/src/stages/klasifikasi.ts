...
const SPOUSE_GROUPS: IdKelompok[] = ['SUAMI', 'ISTRI'];
...
export function klasifikasikanMasalah(masalah: Masalah, konfigurasi: KonfigurasiMadzhab, adaDzawilArham: boolean): HasilKlasifikasi | Unsupported {
  const { ashl, saham, kelompokKelompok } = masalah;
  ...
  if (total > ashl) {
    ...
    return {
      ...
      jejak: [
        ...
        { tahap: 'klasifikasi', refs: ['R09-3', 'R09-4'], jenis: 'AUL', from: ashl, to: total },
      ],
    };
  }
  if (kelompokKelompok.some(isResidueGroup)) throw new Error('invariant: ada ashabah tetapi saham < ashl');
  return applyRadd(masalah, total, konfigurasi, adaDzawilArham);
}

function applyRadd(masalah: Masalah, total: bigint, konfigurasi: KonfigurasiMadzhab, adaDzawilArham: boolean): HasilKlasifikasi | Unsupported {
  const { ashl, saham, kelompokKelompok } = masalah;
  ...
  const pasangan = kelompokKelompok.find(g => SPOUSE_GROUPS.includes(g.id));
  const receivers = kelompokKelompok.filter(g => g !== pasangan);
  ...
  const raddSaham: Record<IdKelompok, bigint> = Object.fromEntries(receivers.map(g => [g.id, saham[g.id]! / divisor]));
  ...
  const finalSaham: Record<IdKelompok, bigint> = { [pasangan.id]: sahamPasangan * pengali };
  for (const g of receivers) finalSaham[g.id] = raddSaham[g.id]! * (sisa / faktor);

  return {
    dasar: hasil, saham: finalSaham, column: 'radd',
    jejak: [
