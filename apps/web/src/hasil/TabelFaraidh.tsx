...
interface Props { hasil: HasilOk | null; ringkasan: RingkasanHasil; sembunyiNominal: boolean }

const uangAtau = (nilai: bigint, sembunyi: boolean) => (sembunyi ? 'Rp ••••••' : formatRupiah(nilai));

export function TabelFaraidh({ hasil, ringkasan, sembunyiNominal }: Props) {
  const { langkah } = useSorot();
  const sorot = (kolom: KolomBab) => (langkah?.kolom === kolom ? 'kolom-sorot' : undefined);
  return hasil ? <TabelBiasa tabel={hasil.tabel} ringkasan={ringkasan} sembunyi={sembunyiNominal} sorot={sorot} />
    : <TabelMunasakhat ringkasan={ringkasan} sembunyi={sembunyiNominal} />;
}

function TabelBiasa({ tabel, ringkasan, sembunyi, sorot }: {
  tabel: TabelMasalah; ringkasan: RingkasanHasil; sembunyi: boolean; sorot: (kolom: KolomBab) => string | undefined;
}) {
      ...
      <tbody>
        {tabel.baris.flatMap(baris => {
          ...
          return anggota.map((id, indeks) => {
            ...
            return (
              <tr key={id} {...pemicu} className={className || undefined} tabIndex={0}>
                ...
                {indeks === 0 && <>
                  <td rowSpan={anggota.length} className={sorot('bagian')}>
                    <span className="bagian-sel">{baris.fardh ? `${baris.fardh.n}/${baris.fardh.d}` : 'Ashabah'}
                      <small>{baris.fardh ? (baris.ashabah ? 'bagian pasti + sisa' : 'bagian pasti') : anggota.length > 1 ? 'sisa, dibagi bersama' : 'sisa'}</small></span>
                  </td>
        ...
        {ringkasan.terhalang.map(orang => <BarisTerhalang key={orang.id} id={orang.id} nama={orang.nama} kelompok={orang.kelompok} alasan={orang.alasan}
...
function TabelMunasakhat({ ringkasan, sembunyi }: { ringkasan: RingkasanHasil; sembunyi: boolean }) {
  const atribut = useAtributOrang();
  return (
    <table className="faraidh">
      <thead>
      ...
      <tbody>
        {ringkasan.penerima.map(orang => {
          ...
          return (
            <tr key={orang.id} {...pemicu} className={className || undefined} tabIndex={0}>
        ...
        {ringkasan.terhalang.map(orang => <BarisTerhalang key={orang.id} id={orang.id} nama={orang.nama} kelompok={orang.kelompok} alasan={orang.alasan} kolom={2} />)}
      </tbody>
    </table>
  );
}

function BarisTerhalang({ id, nama, kelompok, alasan, kolom }: { id: IdOrang; nama: string; kelompok: string; alasan: string; kolom: number }) {
  const atribut = useAtributOrang();
  const { className, ...pemicu } = atribut(id);
  return (
    <tr {...pemicu} className={['mahjub', className].filter(Boolean).join(' ')} tabIndex={0}>
