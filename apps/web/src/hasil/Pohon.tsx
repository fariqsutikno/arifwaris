...
export function Pohon({ graf, ringkasan, urutanWafat, bentuk, sedangMenebak, sembunyiNominal, saatPilih }: Props) {
  ...
  return (
    <div className="pohon-wadah">
      <div className={langkah ? 'pohon redup' : 'pohon'} ref={wadah}>
        ...
        {letak.baris.map((baris, indeksBaris) => (
          <div className="pohon-baris" key={indeksBaris}>
            {baris.map(id => {
              ...
              const nama = namaOrang(graf, ringkasan.statusOrang, id);
              const { className, ...pemicu } = atribut(id);
              const kelas = orang.penghubung ? 'node-orang penghubung'
                : almarhum ? 'node-orang almarhum'
                : dapat && !sedangMenebak ? `node-orang g-${dapat.kelompok}`
                : halang && !sedangMenebak ? 'node-orang putus'
                : `node-orang ${dapat || halang ? `g-${(dapat ?? halang)!.kelompok}` : 'putus'}`;
              return (
                <button key={id} type="button" {...pemicu} className={[kelas, className].filter(Boolean).join(' ')}
                  aria-label={`${nama}. Lihat penjelasan`} onClick={() => saatPilih(id)} disabled={orang.penghubung}>
                  <span className="peran-node">{almarhum ? (id === graf.idPewaris ? 'Pewaris' : 'Wafat sebelum dibagi')
                    : orang.penghubung ? 'Sudah wafat' : halang && !sedangMenebak ? 'Terhalang (mahjub)' : ''}</span>
                  <b>{id === graf.idPewaris ? orang.nama ?? 'Almarhum' : nama}</b>
                  {dapat && !sedangMenebak && (
                    <span className="dapat-node">
                      <span className="frac">{pecahanTeks(dapat.saham, ringkasan.penyebut, bentuk)}</span>
                      <span className="angka">{sembunyiNominal ? 'Rp ••••••' : formatRupiah(dapat.nominal)}</span>
                    </span>
                  )}
                  {halang && !sedangMenebak && <span className="alasan-node">{halang.alasan}</span>}
                  {!dapat && !halang && !almarhum && !orang.penghubung && !sedangMenebak && <span className="alasan-node">Tidak mewarisi</span>}
                </button>
              );
            })}
          </div>
